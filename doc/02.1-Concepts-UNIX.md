# A crash course on what is what wrt computing 

## The 'computer' 

All **turing complete** machines are equivalent and can **simulate** each other. 
Nearly all machines we call computers today are turing complete. 
  * arduino
  * tablet
  * laptop
  * server
Much easier to simulate a smaller computer with a bigger computer (but reverse is also possible). 
When we simulate a smaller computer using a larger one, we call this a **virtual machine**. But, why use a virtual machine? 

  * resource allocation
  * security
  * **reproductibility** = immunity from software and hardware changes

## The prgramming languages 
...
While hardware got cheaper and more widespread, licencing unix became an issue, so many operating systems (OS) emulating its behaviour were written independently. 
Long story short, POSIX standards capture the original unix behaviour and provide a uniform environment across operating systems. 
Even though personal computers are abundant and orders of magnitude faster today, the situation in the bleeding edge is very similiar to 70s; ve have few very powerful computers shared by many researchers for gnarly workloads. 
Such evironments are largely POSIX compliant, so familiarizing yourself with posix behaviour will help you feel at home in any high performance computing environment. 

Note that sometimes I use *nix to refer to all posix compliant operating systems.
...
We as humans have zero direct interaction with the kernel, we interact with the **shell**, which interacts with the **kernel**, which interacts with the **hardware** (through a bios, if you will). 
Claiming we 'use linux', is a bit of a misconception in this sense, but more on this later. 

## Bash is the shell 

There are many ways to interact with the linux kernel and these are called **shell**s. 
Bash is by far the most common you'll find in a linux system, and is fully POSIX compliant. 
...
Sometimes a community will package a collection of software based on the linux kernel, providing a complete and coherent operating system and **repositories** of software. 
Such collections are called 'distributions', and many popular ones are available for free. 
Examples are Debian, Ubuntu, CentOS, Arch, and Mint. 
...
Most of these users are regular users which each belong to a set of groups, with given permissions, but one, namely **root** user is special in that it has unrestricted access to everything.
**The user root is the super user**. 
...
You can prefix your commands with **sudo** to act as the superuser. 
...
Your VM is dead now, no confirmations, no chance of recovery. 
...
Be very careful if you ever get root privilages in real life. 
Be very very careful. 


## The filesystem 

The filesystem has a tree structure, with a singular **root**, that is called /. 
...
The craftful usage of these features are beyond the scope of this class and generally comes with experience but at the very least you need to be aware of their existence. 

==== Permissions ====

In a *nix filesystem, each file has an **owner** and **permissions**, you have to familiarize yourself with these as they can be confusing or get you in trouble in mutiple ways. 
...
//How about these? Can superuser write these files?//
...
This requires no additional software in a posix compliant system such as linux or MacOS and can be done entirely using native features. 
...
A lot of programs have a default behaviour they demonstrate when executed with no parameters, but some require parameters. 
...
It is ok to ask naive questions sometimes, but if you go around a forum and start asking the simplest principals of a program directly to its community, you will be told to RTFM. 
...
Each program takes an input and generates an output, this is an essential feature of *nix programming philosophy that allows us to daisychain programs into workflows. 
...
Thinking about programs in such modular fashion and connecting them robustly together is a most fundamental skill you need to develop. 
...
Although there are very fancy things you can learn to do with process management, these three tricks should take you a long way. 
